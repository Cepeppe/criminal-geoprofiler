...
- **Interfaccia bilingue** italiano/inglese, con rilevamento automatico dalla lingua del browser.
...
L'interfaccia è disponibile in **italiano** e in **inglese**. La lingua si sceglie dal selettore
`IT / EN` in testa al pannello, oppure da *Vista → Interfaccia → Lingua*, dove è disponibile anche
l'opzione «Segui il browser» (predefinita). La scelta è salvata in `localStorage`.

Un link può forzare la lingua con il parametro `?lang=it` o `?lang=en`; il valore diventa la
preferenza salvata, così un link condiviso in inglese resta in inglese anche dopo il ricaricamento.

Il cambio è immediato e non ricarica la pagina: oltre ai testi vengono riallineati la formattazione
numerica (`it-IT` / `en-GB`), le date delle etichette del dataset e i metadati della pagina
(`<html lang>`, `<title>`, Open Graph). Le etichette scritte o importate dall'utente non vengono
toccate. Le stringhe vivono tutte in [`js/i18n.js`](js/i18n.js): aggiungere una lingua significa
aggiungere un dizionario a quel file.
...
└─ js/
   ...
   ├─ i18n.js            dizionari italiano/inglese e applicazione al DOM
